...
func TestRegistrationGrantsOnlyUserAndRollsBackWithoutRole(t *testing.T) {
	...
	require.Equal(t, 1, u.Gender)
	require.NoError(t, tx.Model(&rbacmodel.Role{}).Where("code=?", "user").Update("status", 1).Error)
	name := "reg-" + uuid.NewString()[:12]
	_, err = svc.Register(context.Background(), &dto.RegisterRequest{Username: name, Password: "ValidPass123", Email: "roll@example.test", Gender: 1}, "")
	require.Error(t, err)
	var count int64
