...
func DefaultWeightConfig() model.VoteWeightConfig {
	return model.VoteWeightConfig{Weights: map[string]float64{"president": 2, "vice_president": 1, "center_director": 1, "minister": 0.5, "vice_minister": 0.5, "deputy": 0.5, "officer": 0.25}, DefaultWeight: 0}
}
...
func validateWeightConfig(cfg model.VoteWeightConfig) error {
	...
	if !valid(cfg.DefaultWeight) || cfg.Weights == nil {
		return response.NewError(response.CodeBadRequest, "默认权重须为非负数，最多两位小数，且不大于 999999.99")
	}
	for code, w := range cfg.Weights {
		if strings.TrimSpace(code) == "" || !valid(w) {
			return response.NewError(response.CodeBadRequest, "职务权重须为非负数，最多两位小数，且不大于 999999.99")
		}
	}
