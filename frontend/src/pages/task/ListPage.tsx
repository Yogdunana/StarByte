...
export default function ListPage() {
    ...
    try {
      const [res, summary] = await Promise.all([
        getTaskList({
          ...
          keyword,
          sort_by: sortBy,
          sort_order: sortBy === 'due_date' ? 'asc' : 'desc',
        }),
        getTaskStats(),
      ]);
  ...
  const remove = (task: Task) =>
    modal.confirm({
      title: tx('删除这项任务？'),
      content: tx('任务将移出日常列表，已有评论和流转记录会保留。存在子任务时，请先处理子任务。'),
      okText: tx('删除任务'),
      okButtonProps: { danger: true },
      cancelText: tx('保留任务'),
      onOk: async () => {
        await deleteTask(task.id);
        message.success(tx('任务已删除'));
        await load();
      },
    });
  return (
    <>
      ...
      <PageIntro
        ...
        title={tx('任务协作')}
        description={tx('分配负责人，跟进进度，保留每一次交接。')}
        actions={
          <>
            {canCreate && (
              <Button
      ...
      {stats && (
        <div className={styles.metrics}>
          ...
          <div>
            <span>{tx('进行中')}</span>
            <strong>{stats.by_status.doing || 0}</strong>
          </div>
          <div>
            <span>{tx('已完成')}</span>
            <strong>{stats.by_status.done || 0}</strong>
          </div>
          <div>
            <span>{tx('需要关注 · 超期')}</span>
            <strong className={styles.overdue}>{stats.overdue}</strong>
          </div>
        </div>
      )}
      <Card>
        <div className={styles.filters}>
          ...
          <Select
            ...
            options={Object.entries(TaskStatusMap).map(([k, v]) => ({
              value: Number(k),
              label: v.text,
            }))}
          />
          <Select
            aria-label={tx('任务优先级')}
            allowClear
            placeholder={tx('全部优先级')}
            value={priority}
            onChange={(v) => {
              setPriority(v);
              setPage(1);
            }}
            options={Object.entries(TaskPriorityMap).map(([k, v]) => ({
              value: Number(k),
              label: v.text,
            }))}
          />
          <Select
            ...
            options={[
              { value: 'created_at', label: tx('最近创建') },
              { value: 'due_date', label: tx('按截止时间') },
              { value: 'priority', label: tx('优先级从高到低') },
            ]}
          />
        </div>
        {failed && (
          <Alert
            type="error"
            showIcon
            message={tx('任务加载失败，当前内容可能不是最新状态')}
        ...
        <TaskCollection
          ...
          actions={(t) => (
            <>
              ...
              {t.can_delete && (
                <Button size="small" type="text" danger onClick={() => remove(t)}>
                  {tx('删除')}
                </Button>
              )}
            </>
      ...
      <FormModal
        ...
        onSubmit={async (values) => {
          ...
          message.success(editing ? tx('修改已保存') : tx('任务已创建'));
          setOpen(false);
          await load();
        }}
      />
