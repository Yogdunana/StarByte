...
const ProfileDetailDrawer: React.FC<Props> = ({ open, profileId, onClose, onSaved }) => {
  ...
  return (
    <Drawer title={profile?.real_name || tx('人员档案')} width={640} open={open} onClose={onClose}>
      {profile && (
        <Tabs
          items={[
            {
              ...
              children: (
                <Descriptions column={1} bordered size="small">
                  <Descriptions.Item label={tx('学号')}>{profile.student_no}</Descriptions.Item>
                  <Descriptions.Item label={tx('性别')}>
                    <StatusTag status={profile.gender} mapping={GenderMap} />
                  </Descriptions.Item>
                  <Descriptions.Item label={tx('年级')}>{profile.grade || '-'}</Descriptions.Item>
                  <Descriptions.Item label={tx('专业')}>{profile.major || '-'}</Descriptions.Item>
                  <Descriptions.Item label={tx('部门')}>
                    {profile.department?.name || '-'}
                  </Descriptions.Item>
                  <Descriptions.Item label={tx('职位')}>
                  ...
                  <Descriptions.Item label={tx('电话')}>
                    {profile.contact_phone || '-'}
                  </Descriptions.Item>
                  <Descriptions.Item label={tx('邮箱')}>
            ...
            },
            {
              ...
              children: (
                <Form form={form} layout="vertical" disabled={!canUpdate}>
                  <Form.Item name="real_name" label={tx('姓名')}>
                    <Input />
                  </Form.Item>
                  ...
                  <Form.Item name="grade" label={tx('年级')}>
                    <Input />
                  </Form.Item>
                  <Form.Item name="major" label={tx('专业')}>
                    <Input />
                  </Form.Item>
                  <Form.Item name="skills" label={tx('技能')}>
                    <Select mode="tags" />
                  </Form.Item>
                  <Form.Item name="bio" label={tx('简介')}>
                    <Input.TextArea rows={4} />
                  </Form.Item>
                  <Form.Item name="contact_phone" label={tx('电话')}>
                    <Input />
                  </Form.Item>
                  <Form.Item name="contact_email" label={tx('邮箱')}>
                    <Input />
                  </Form.Item>
                  {canUpdate && (
            ...
            },
            {
              ...
              children: (
                <Table
                  ...
                  pagination={false}
                  columns={[
                    { title: tx('项目'), dataIndex: 'name' },
                    { title: tx('角色'), dataIndex: 'role' },
                    { title: tx('周期'), dataIndex: 'period' },
                  ]}
                />
              ),
            },
            {
              ...
              children: (
                <Table
                  ...
                  pagination={false}
                  columns={[
                    { title: tx('字段'), dataIndex: 'field_name', width: 120 },
                    { title: tx('旧值'), dataIndex: 'old_value' },
                    { title: tx('新值'), dataIndex: 'new_value' },
                    { title: tx('原因'), dataIndex: 'reason', width: 120 },
                    { title: tx('时间'), dataIndex: 'created_at', width: 180 },
                  ]}
                />
              ),
            },
