...
import { getApplicationHistory, resubmitApplication } from '@/api/member';
...
const ReviewDrawer: React.FC<ReviewDrawerProps> = ({ open, record, mode, onClose, onDone }) => {
  ...
  return (
    <Drawer
      title={record ? tx('{{value0}} 的申请', { value0: record.real_name }) : tx('申请详情')}
      width={640}
        ...
        <>
          <Descriptions column={1} size="small" bordered>
            <Descriptions.Item label={tx('类型')}>
              {record.applicant_type === 2 ? tx('干事') : tx('会员')}
            </Descriptions.Item>
            <Descriptions.Item label={tx('学号')}>{record.student_no}</Descriptions.Item>
            ...
            <Descriptions.Item label={tx('理由')}>{record.reason}</Descriptions.Item>
            <Descriptions.Item label={tx('经历')}>{record.experience || '-'}</Descriptions.Item>
          ...
          record.admission_stage !== 'probation' ? (
            ...
            <>
              ...
              <AdmissionPanel
                key={record.id}
                id={record.id}
                officer={record.applicant_type === 2}
          ...
          {mode === 'resubmit' && (
            <Form form={form} layout="vertical" style={{ marginTop: 16 }} initialValues={record}>
              ...
              <Form.Item name="student_no" label={tx('学号')}>
                <Input />
              </Form.Item>
              <Form.Item name="contact_phone" label={tx('联系电话')}>
                <Input />
              </Form.Item>
              <Form.Item name="contact_email" label={tx('邮箱')} rules={[{ type: 'email' }]}>
                <Input />
              </Form.Item>
              ...
              <Form.Item name="reason" label={tx('申请理由')}>
                <Input.TextArea rows={3} />
              </Form.Item>
              <Button
                type="primary"
                loading={loading}
                onClick={() => {
                  void handleResubmit().catch(() => undefined);
                }}
              >
                {tx('重新提交')}
              </Button>
