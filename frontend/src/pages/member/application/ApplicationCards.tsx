  ...
  if (!rows.length)
    return (
      <Empty
        ...
        description={mine ? tx('提交申请后，在这里查看每一步进度') : tx('没有符合条件的申请')}
      />
    );
  return (
      ...
      {rows.map((row) => (
        <article key={row.id} className={styles.card}>
          <div className={styles.heading}>
            <h3>{row.real_name}</h3>
            <Tag>{row.applicant_type === 2 ? tx('干事申请') : tx('会员申请')}</Tag>
          </div>
          <p>
            {row.department_name || tx('未选择部门')} · {row.student_no}
          </p>
          ...
          <Space wrap>
            ...
            {review &&
              row.status !== 4 &&
              (row.status !== 3 || row.admission_stage === 'probation') && (
                <Button type="primary" onClick={() => onOpen(row, 'review')}>
                  {tx('处理申请')}
                </Button>
              )}
            {mine && row.status === 5 && !row.historical_review_required && (
              <Button type="primary" onClick={() => onOpen(row, 'resubmit')}>
                {tx('补充材料')}
              </Button>
            )}
          </Space>
