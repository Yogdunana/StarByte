...
const ListPage: React.FC = () => {
  ...
  const columns: ColumnsType<Internship> = [
    ...
    { title: tx('成员'), key: 'user', width: 100, render: (_, r) => r.user?.name || '-' },
    { title: tx('单位'), dataIndex: 'organization' },
    {
      title: tx('类型'),
      dataIndex: 'type',
      width: 100,
      render: (v: number) => <StatusTag status={v} mapping={InternshipTypeMap} />,
    },
    {
      title: tx('时长'),
    ...
    },
    {
      ...
      dataIndex: 'status',
      width: 90,
      render: (v: number) => <StatusTag status={v} mapping={InternshipStatusMap} />,
    },
    {
      title: tx('操作'),
      width: 150,
        ...
        <Space>
          ...
          {canDelete && (
            <Button
              ...
              danger
              onClick={() => {
                void deleteInternship(record.id).then(() => {
  ...
  return (
    <div>
      <div className="intern-hero">
        <div>
          <h2>{tx('IT 实习档案')}</h2>
          <p>{tx('登记校内社团与校外实习，按时长统计、排名，方便招新后的干事培养。')}</p>
        </div>
        {canCreate && (
          <Button
            type="primary"
      ...
      <Card className="page-shell">
        {canConfig && config && (
          <Space style={{ marginBottom: 16 }} wrap>
            <span>{tx('学生可改')}</span>
            <Switch
              checked={config.allow_student_edit}
              onChange={(v) => {
                void updateInternshipConfig({ allow_student_edit: v }).then(setConfig);
              }}
            />
            <span>{tx('部长/副部长可改')}</span>
            <Switch
              checked={config.allow_minister_edit}
              onChange={(v) => {
        ...
        <Space style={{ marginBottom: 16 }} wrap>
          ...
          <Select
            ...
            options={Object.entries(InternshipStatusMap).map(([k, v]) => ({
              value: Number(k),
              label: v.text,
            }))}
          />
