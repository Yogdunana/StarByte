...
const CheckinPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([getActivityDetail(activityId), getMyRegistration(activityId)])
      .then(([a, reg]) => {
        setActivity(a);
        setMine(reg);
      })
  ...
  if (done) return <Result status="success" title={tx('签到成功')} subTitle={activity?.title} />;

  return (
    <Card title={tx('活动签到')}>
      {!activity ? (
        <Result status="info" title={tx('未指定活动')} />
      ) : !mine || mine.status !== 1 ? (
        <Result status="warning" title={tx('你尚未通过该活动报名')} subTitle={activity.title} />
      ) : mine.checkin_status === 1 ? (
        <Result status="success" title={tx('你已签到')} subTitle={activity.title} />
      ) : (
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          ...
          <Input
            placeholder={tx('扫码后自动填入，或粘贴签到令牌')}
            value={token}
            onChange={(e) => setToken(e.target.value)}
          />
          <Button type="primary" onClick={() => void doCheckin()}>
            {tx('确认签到')}
          </Button>
        </Space>
      )}
