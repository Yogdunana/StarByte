...
const FillPage: React.FC = () => {
  ...
  if (!form) return <Result status="404" title={tx('表单不存在')} />;
  if (form.status !== 1)
    return (
      <Result
        status="warning"
        title={tx('表单未发布')}
        extra={<a onClick={() => nav('/forms')}>{tx('返回列表')}</a>}
      />
    );

  return (
    <Card title={form.name} extra={<a onClick={() => nav('/forms')}>{tx('返回')}</a>}>
