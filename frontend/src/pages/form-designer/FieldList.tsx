...
const FieldList: React.FC<Props> = ({
  ...
  return (
    ...
    >
      {fields.length === 0 ? <Empty description={tx('从左侧拖入或点击添加字段')} /> : null}
      {fields.map((f, i) => (
        <div
          key={f.name}
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
            <span>
              {f.label || f.name} <Tag>{FIELD_TYPE_LABELS[f.type]}</Tag>
              {f.required ? <Tag color="red">{tx('必填')}</Tag> : null}
            </span>
