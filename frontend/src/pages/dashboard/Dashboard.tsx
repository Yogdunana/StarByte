...
export default function Dashboard() {
  ...
  const {
    ...
    failed,
    reload,
  } = useWorkspace(canReadStats, announcementFeed.enabled, !announcementFeed.loading);
  const locale =
  ...
  const metrics = [
    {
      ...
      value: approvalTotal,
      caption: t('dashboard.metricApprovalsHint'),
      path: '/workflow/todo',
    },
    {
      label: t('dashboard.metricTasks'),
      value: taskTotal,
      caption: t('dashboard.metricTasksHint'),
      path: '/task/my',
    },
    {
      label: t('dashboard.metricInterviews'),
      value: failed.includes('interviews') ? null : interviews.length,
      caption: t('dashboard.metricInterviewsHint'),
      path: '/interview/my',
    },
    {
      label: t('dashboard.metricUnread'),
      value: unread,
      caption: t('dashboard.metricUnreadHint'),
      path: '/notification/list',
    },
  ];
  const failedLabels = FAIL_KEYS.filter((key) => failed.includes(key)).map((key) =>
    t(`dashboard.failed.${key}`),
  );
  return (
    <motion.div className={styles.page} variants={staggerEnter} initial="hidden" animate="show">
      <motion.section
        className={styles.hero}
        aria-labelledby="workspace-greeting"
        variants={fadeUp}
      >
        <div className={styles.heroCopy}>
          ...
          <div className={styles.heroActions}>
            <Link to="/task/my" className={styles.heroLink}>
              {t('dashboard.viewTasks')} <ArrowRightOutlined />
            </Link>
            <Button
      ...
      {failedLabels.length > 0 && (
        <motion.div variants={fadeUp}>
          <Alert
            ...
            showIcon
            type="warning"
            message={t('dashboard.loadPartial', {
              parts: failedLabels.join(t('dashboard.partSep')),
            })}
          />
        </motion.div>
      )}
      ...
      <motion.div className={styles.workspace} variants={staggerEnter}>
        <motion.section className={styles.queue} aria-labelledby="queue-title" variants={fadeUp}>
          <div className={styles.sectionHeader}>
            <div>
              <span className={styles.eyebrow}>{t('dashboard.reviewEyebrow')}</span>
              <h2>{t('dashboard.reviewTitle')}</h2>
            </div>
            <Link to="/workflow/todo">
              {t('dashboard.allApprovals')} <ArrowRightOutlined />
            </Link>
          </div>
          <Card className={styles.approvalCard}>
            {loading ? (
              <Skeleton active paragraph={{ rows: 2 }} />
            ) : failed.includes('approvals') ? (
              <EmptyState
                title={t('dashboard.emptyApprovalsFail')}
                description={t('empty.retryHint')}
              />
            ) : approvals.length === 0 ? (
              <EmptyState
                icon={<CheckOutlined />}
                title={t('dashboard.emptyApprovals')}
                description={t('dashboard.emptyApprovalsHint')}
              />
            ) : (
              approvals.map((item) => (
                <Link
                  ...
                  to={`/workflow/todo?task_id=${encodeURIComponent(item.id)}`}
                >
                  ...
                  <div className={styles.rowContent}>
                    ...
                    <p>
                      ...
                      {item.due_date
                        ? ` · ${t('dashboard.due')} ${formatDateTime(item.due_date, 'MM-DD HH:mm')}`
                        : ''}
                    </p>
                  </div>
                  <ArrowRightOutlined />
          ...
          <div className={styles.sectionHeader}>
            <div>
              <span className={styles.eyebrow}>{t('dashboard.focusEyebrow')}</span>
              <h2 id="queue-title">{t('dashboard.focusTitle')}</h2>
            </div>
            <Link to="/task/my">
              {t('dashboard.allTasks')} <ArrowRightOutlined />
            </Link>
          </div>
          <Card className={styles.taskCard}>
            {loading ? (
              <Skeleton active paragraph={{ rows: 4 }} />
            ) : failed.includes('tasks') ? (
              <EmptyState
                title={t('dashboard.emptyTasksFail')}
                description={t('empty.retryHint')}
              />
            ) : tasks.length === 0 ? (
              <EmptyState
                ...
                action={
                  <Link to="/task/my">
                    {t('dashboard.viewTaskHistory')} <ArrowRightOutlined />
                  </Link>
                }
              />
            ) : (
                ...
                {tasks.map((task, index) => (
                  <div key={task.id} className={styles.taskRow}>
                    ...
                    <div className={styles.rowContent}>
                      ...
                      <p>
                        {task.due_date
                          ? `${t('dashboard.due')} ${formatDateTime(task.due_date, 'MM-DD HH:mm')}`
                          : t('dashboard.noDue')}
                      </p>
                    </div>
                    <Tag color={task.status === 1 ? 'processing' : 'default'}>
                      {task.status === 1
                        ? t('dashboard.taskDoing')
                        : task.status === 4
                          ? t('dashboard.taskHeld')
                          : t('dashboard.taskTodo')}
                    </Tag>
                  </div>
                ))}
              </div>
          ...
          <div className={styles.sectionHeader}>
            <div>
              <span className={styles.eyebrow}>{t('dashboard.nextEyebrow')}</span>
              <h2>{t('dashboard.nextTitle')}</h2>
            </div>
            <Link to="/interview/my">
              {t('dashboard.viewSchedule')} <ArrowRightOutlined />
            </Link>
          </div>
          <Card className={styles.scheduleCard}>
            {loading ? (
              <Skeleton active paragraph={{ rows: 2 }} />
            ) : failed.includes('interviews') ? (
              <EmptyState
                title={t('dashboard.emptyInterviewsFail')}
                description={t('empty.retryHint')}
              />
            ) : interviews.length === 0 ? (
              <EmptyState
                icon={<CalendarOutlined />}
                title={t('dashboard.emptyInterviews')}
                description={t('dashboard.emptyInterviewsHint')}
              />
            ) : (
              interviews.slice(0, 3).map((item) => (
                <div className={styles.taskRow} key={item.id}>
                  ...
                  <div className={styles.rowContent}>
                    ...
                    <p>
                      {item.scheduled_time
                        ? formatDateTime(item.scheduled_time, 'MM-DD HH:mm')
                        : t('dashboard.timeTbd')}{' '}
                      · {item.location || t('dashboard.placeTbd')}
                    </p>
                  </div>
                  <Tag>
                    {item.status === 2
                      ? t('dashboard.ivDoing')
                      : item.status === 1
                        ? t('dashboard.ivChecked')
                        : t('dashboard.ivPending')}
                  </Tag>
                </div>
              ))
            )}
        ...
        <motion.aside className={styles.aside} variants={fadeUp}>
          {announcementFeed.enabled && (
            <section className={styles.feed} aria-labelledby="announcement-feed-title">
              ...
              {loading ? (
                ...
                announcements.map((item) => (
                  <Link key={item.id} className={styles.feedRow} to={`/announcement/${item.id}`}>
                    <span>
                      {item.pinned ? `${t('dashboard.feedPinned')} · ` : ''}
                      {item.title}
                    </span>
                    <ArrowRightOutlined />
                  </Link>
              ...
              <Link to="/announcement/list">
                {t('dashboard.feedAll')} <ArrowRightOutlined />
              </Link>
            </section>
          )}
          <section className={styles.quickAccess}>
            ...
            <div className={styles.shortcuts}>
              <Link to="/member/application">
                <FileTextOutlined />
                <span>{t('dashboard.shortcutApply')}</span>
                <ArrowRightOutlined />
              </Link>
              <Link to="/interview/my">
                <ScheduleOutlined />
                <span>{t('dashboard.shortcutInterview')}</span>
                <ArrowRightOutlined />
              </Link>
              <Link to="/internship/my">
                <TeamOutlined />
                <span>{t('dashboard.shortcutInternship')}</span>
                <ArrowRightOutlined />
              </Link>
              {announcementFeed.enabled && (
                <Link to="/announcement/list">
                  <NotificationOutlined />
                  <span>{t('dashboard.shortcutAnnounce')}</span>
                  <ArrowRightOutlined />
                </Link>
              )}
              {canReadTasks && (
                <Link to="/task/board">
                  <CheckOutlined />
                  <span>{t('dashboard.shortcutBoard')}</span>
                  <ArrowRightOutlined />
                </Link>
              )}
              {canReadFiles && (
                <Link to="/files">
                  <FolderOutlined />
                  <span>{t('dashboard.shortcutFiles')}</span>
                  <ArrowRightOutlined />
                </Link>
              )}
            </div>
          ...
          <section className={styles.application}>
            ...
            {loading ? (
              <Skeleton active paragraph={{ rows: 2 }} />
            ) : failed.includes('applications') ? (
              <p>{t('dashboard.journeyFail')}</p>
            ) : applications.length ? (
              applications.slice(0, 2).map((item) => (
                <div key={item.id} className={styles.applicationRow}>
                  <span>
                    {item.applicant_type === 2
                      ? t('dashboard.officerApply')
                      : t('dashboard.memberApply')}
                  </span>
                  <Tag>
                    {item.current_stage ||
                      applicationStates[item.status] ||
                      t('dashboard.appPending')}
                  </Tag>
                </div>
              ))
            ) : (
      ...
      {canReadStats && overview && (
        <motion.section className={styles.overview} variants={fadeUp}>
          <div>
            <span className={styles.eyebrow}>{t('dashboard.assocEyebrow')}</span>
            <h2>{t('dashboard.assocTitle')}</h2>
          </div>
          <div>
          ...
          <div>
            <strong>{overview.total_tasks_in_progress}</strong>
            <span>{t('dashboard.inProgress')}</span>
          </div>
          <div>
            <strong>{overview.total_meetings_this_month}</strong>
            <span>{t('dashboard.meetings')}</span>
          </div>
          <Link to="/stats/overview">
            {t('dashboard.viewStats')} <ArrowRightOutlined />
          </Link>
        </motion.section>
      )}
    </motion.div>
