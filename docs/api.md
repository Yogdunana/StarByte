...
- 入会：`/member/applications`、审核 approve/reject/supplement/transfer（新申请走可配置 `member_application`：干事→部长→会长，`GET .../progress` 看进度）、`/member/profiles`
...
- 请假：`GET|POST /leave`、`GET /leave/my`、`GET /leave/types`、`POST /leave/types`、`PUT /leave/types/:id`、`GET /leave/balance`、`GET /leave/stats`、`GET /leave/calendar`、`GET /leave/todos`、`PUT /leave/:id/approve|reject`；新申请走 `leave_approval` 流程实例（部长→会长），历史记录无实例时单级回退
